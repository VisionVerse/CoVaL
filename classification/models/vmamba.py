...
class Linear2d(nn.Linear):
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys, error_msgs):
        # Check if weight exists and has compatible size
        weight_key = prefix + "weight"
        if weight_key in state_dict:
            ckpt_weight = state_dict[weight_key]
            # Only reshape if the total number of elements matches
            if ckpt_weight.numel() == self.weight.numel():
                state_dict[weight_key] = ckpt_weight.view(self.weight.shape)
            else:
                # Size mismatch: remove from state_dict to skip loading
                state_dict.pop(weight_key)
                if strict:
                    error_msgs.append(f'size mismatch for {weight_key}: '
                                    f'copying a param with shape {ckpt_weight.shape} from checkpoint, '
                                    f'the shape in current model is {self.weight.shape}.')
...
class PatchMerging2D(nn.Module):
    def __init__(self, dim, out_dim=-1, norm_layer=nn.LayerNorm, channel_first=False):
        super().__init__()
        self.dim = dim
        # Note: channel_first is accepted for compatibility but not used in PatchMerging2D
        # because it always operates in channel_last format (B, H, W, C)
